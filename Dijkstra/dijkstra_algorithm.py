...
class Dijkstra:
    ...
    def dijkstra(self):
        ...
        self.plot_boundary()
        index = self.calculate_index(self.start[0], self.start[1])
        init_node = Node()
        ...
        for motion_x, motion_y, cost in self.motion:
            ...
            if target_x == self.end[0] and target_y == self.end[1]:
                break
            point_index_before = self.calculate_index(current_x, current_y)
            init_node.cost = cost + self.CloseList[point_index_before].cost
            init_node.father_index = [current_x, current_y]
            point_index_current = self.calculate_index(target_x, target_y)
    ...
    def get_path(self):
        ...
        for ii in self.searched_space:
            ...
            if count % 30 == 0:
                plt.pause(0.001)
        xy = [endPoint]
        end_index = self.calculate_index(endPoint[0], endPoint[1])
        xy.append(self.CloseList[end_index].father_index)
        cc = self.CloseList[end_index].father_index
        xy.append(cc)
        while 1:
            end_index = self.calculate_index(cc[0], cc[1])
            cc = self.CloseList[end_index].father_index
            xy.append(cc)
            if self.start == cc:
                return xy
